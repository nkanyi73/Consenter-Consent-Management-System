<template>
  <v-container>
    <v-card>
      <v-container fluid>
        <v-row dense>
          <v-col v-for="(requester, index) in requesters" :key="requester" :cols="12">
            <v-card>
              <v-card-title
                >User at Address &lt; {{ requester }} &gt;
              </v-card-title>
              <v-card-text
                >has access to your {{category[index]}}</v-card-text
              >
              ...
              <v-card-actions>
                ...
                <v-btn
                  elevation="5"
                  @click="revoke(requester, category_ids[index], notification_ids[index])"
                  >Revoke</v-btn
                >
              </v-card-actions>
            </v-card>
...
export default {
  ...
  methods: {
    async fetchData() {
      setTimeout(async () => {
        ...
        const notifications = await axios.get(
          `${NODE_API_URL}/notifications?to=${formattedAddress}&status=Accepted`
        );
    ...
    async revoke(requester, category_id, notification_id) {
